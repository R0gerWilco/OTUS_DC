...
Нет у меня объяснения и тому факту, что один из LEAFов vPC пары видит ARP-запись клиентского узла  VLAN, а его сосед  - через peer-link, и при отключении   connected порта на первом лифе МАС пропадает, а второй все также видит клиентский МАС через свой peer-link, где его быть уже не должно
