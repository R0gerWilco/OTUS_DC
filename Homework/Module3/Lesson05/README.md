...
Потери продолжались вплоть до того момента, пока LEAF, который ранее изучил данные клиента из ARP через Peer-link, не перестроил свою ARP-таблицу на изучение от connected подключения. В дампе, собираемом на аплинках клиента, половина ICMP reply все равно упорно приходит на отключенный на клиенте порт.

Грешу особенности поведения коммутаторов в виртуализации, но проверить на железе нет возможности. Траблшутинг не помог, причину такого поведения выяснить не удалось.
