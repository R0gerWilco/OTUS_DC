...
- Конфигурация Underlay/Overlay протоколов не менялась по сравнению с [прошлым ДЗ для L3VNI](https://github.com/R0gerWilco/OTUS_DC/blob/main/Homework/Module3/Lesson03/README.md)
...
Нет у меня объяснения и тому факту, что один из LEAFов vPC пары видит ARP-запись клиентского узла через connected сегмент, а его сосед  - через peer-link.

При отключении   connected порта на первом лифе  ARP-запись клиента пропадает, а второй LEAF  все также видит клиента через свой peer-link, где его быть уже не должно.

Траффик по крайней мере ICMP не восстанавливается.

ARP-таблица при полноценной подключении клиента на оба LEAFа выглядит так:
...
```
После отключения на клиентском устройстве порта Gi0/1, куда при полноценном агрегированном канале хеш-функция  LACP протокола на стороне клиента упорно распределяла все ICMP request и куда приходила половина ICMP reply, начались потери.

Потери продолжались вплоть до того момента, пока LEAF, который ранее изучил данные клиента из ARP через Peer-link, не перестроил свою ARP-таблицу на изучение от connected подключения. 
В дампе, собираемом на аплинках клиента, во время потерь траффика половина ICMP reply все равно упорно приходит на отключенный на клиенте порт.

Грешу на особенности поведения коммутаторов в виртуализации, но проверить на железе нет возможности. Траблшутинг не помог, причину такого поведения выяснить не удалось.
