...
При отключении   connected порта на первом лифе МАС пропадает, а второй все также видит клиентский МАС через свой peer-link, где его быть уже не должно.
...
Пришлось проверять отказоустойчивость, отключая порт непосредственно на самом клиентском устройстве:
```bash
